...
export default function SavedSets({ letters, comboLang, letterCount, onRestore, uiLang = 'en' }) {
  ...
  const save = () => {
    ...
    const entry = { id: Date.now(), name: label, letters, comboLang, letterCount };
    setSets(prev => [entry, ...prev.slice(0, 9)]);
    setNaming(false);
    setName('');
  };

  const remove = (id) => setSets(prev => prev.filter(s => s.id !== id));
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display font-semibold text-sm" style={{ color: 'var(--text)' }}>
            {isAr ? 'المجموعات المحفوظة' : 'Saved Sets'}
          </h3>
        </div>
        <button
          onClick={() => { setNaming(n => !n); setName(''); }}
      ...
      {sets.length === 0 ? (
        <div className="text-center py-4">
          ...
          <p className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>
            {isAr ? 'لا توجد مجموعات محفوظة' : 'No saved sets yet'}
          </p>
        </div>
      ) : (
        <div className="space-y-1.5">
          {sets.map(s => (
            <div key={s.id}
              className="flex items-center gap-2 px-3 py-2 rounded-lg group transition-all hover:opacity-90"
              ...
              <button onClick={() => onRestore(s)} className="flex-1 min-w-0 text-left">
                ...
                <p className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>
                  {s.letters.join(' ')} · r={s.letterCount}
                </p>
              </button>
