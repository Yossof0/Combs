...
import { Search, Copy, Download, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react';
const PAGE_SIZE = 60;
const SORT_OPTIONS = [
  { value: 'default', label: 'Default' }, { value: 'az', label: 'A → Z' },
  { value: 'za', label: 'Z → A' }, { value: 'len-asc', label: 'Length ↑' },
  { value: 'len-desc', label: 'Length ↓' }, { value: 'real-first', label: 'Real words first' },
];
export default function ResultsPanel({ results, realWords, realOnly=false, isLoading, wordCheckProgress, language, uiLang='en' }) {
  ...
  const [page, setPage] = useState(1); const [copied, setCopied] = useState(false);
  const isArabic = language === 'ar';
  const filtered = useMemo(() => {
    let r = results;
    if (realOnly) r = r.filter(w => realWords.has(w));
    if (search.trim()) { const q = search.trim().toLowerCase(); r = r.filter(w => w.toLowerCase().includes(q)); }
    switch(sort) {
      ...
      case 'real-first': return [...r].sort((a,b) => (realWords.has(a)?0:1)-(realWords.has(b)?0:1));
      default: return r;
    }
  ...
  const exportTxt = () => { const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([filtered.join('\n')],{type:'text/plain'})); a.download=`combinations-${Date.now()}.txt`; a.click(); };
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border:'1px solid var(--border)', background:'var(--surface)' }}>
      <div className="p-3 border-b flex flex-wrap gap-2 items-center" style={{ borderColor:'var(--border)' }}>
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="text-xs font-mono flex-shrink-0">
            ...
            <span style={{ color:'var(--text-muted)' }}>/{results.length.toLocaleString()}</span>
          </div>
          {realWords.size > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded text-xs font-display"
              style={{ background:'rgba(34,197,94,0.08)', border:'1px solid rgba(34,197,94,0.3)', color:'#22c55e' }}>
              <BookOpen size={10}/>{realWords.size} real
            </span>
          )}
        </div>
        <div className="relative">
          <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color:'var(--text-muted)' }}/>
          <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} placeholder="Filter..." dir={isArabic?'rtl':'ltr'}
            className="pl-7 pr-3 py-1.5 rounded-lg text-xs font-mono outline-none"
            style={{ background:'var(--surface2)', border:'1px solid var(--border)', color:'var(--text)', width:'130px' }}/>
        </div>
      ...
      {wordCheckProgress && wordCheckProgress.done < wordCheckProgress.total && (
        <div className="px-3 py-2 border-b" style={{ borderColor:'var(--border)' }}>
          <div className="flex justify-between text-xs font-mono mb-1" style={{ color:'var(--text-muted)' }}>
            <span>Checking dictionary...</span><span>{wordCheckProgress.done}/{wordCheckProgress.total}</span>
          </div>
          <div className="h-1 rounded-full overflow-hidden" style={{ background:'var(--surface2)' }}>
            <div className="h-full rounded-full transition-all" style={{ width:`${(wordCheckProgress.done/wordCheckProgress.total)*100}%`, background:'var(--brand)' }}/>
          </div>
        </div>
      )}
      <div className="p-3">
        {isLoading ? (
          ...
          <div className="grid gap-1.5" style={{ gridTemplateColumns:'repeat(auto-fill, minmax(70px, 1fr))' }} dir={isArabic?'rtl':'ltr'}>
            {paginated.map((word,i) => {
              ...
              return (
                <div key={`${word}-${i}`}
                  ...
                  {word}{isReal&&<span className="block text-green-400 text-[8px] leading-none mt-0.5">✓</span>}
                </div>
              );
            })}
          </div>
      ...
      {totalPages > 1 && (
        <div className="px-3 pb-3 flex items-center justify-between border-t pt-3" style={{ borderColor:'var(--border)' }}>
          <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-display disabled:opacity-30 transition-all hover:scale-105"
            style={{ background:'var(--surface2)', border:'1px solid var(--border)', color:'var(--text)' }}>
          ...
          <div className="flex items-center gap-1">
            {Array.from({length:Math.min(5,totalPages)},(_,i)=>{
              ...
              return <button key={p} onClick={()=>setPage(p)}
                ...
                style={{ background:page===p?'var(--brand)':'var(--surface2)', border:'1px solid var(--border)', color:page===p?'#fff':'var(--text)', fontWeight:page===p?700:400 }}>{p}</button>;
            })}
          </div>
